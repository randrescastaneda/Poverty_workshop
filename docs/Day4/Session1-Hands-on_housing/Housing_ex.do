...
sum rent [aw = weight], detail

gen lnrent = ln(rent)

levelsof subregion, local(regions)
foreach region of local regions {
	qui sum rent   [aw = weight] if renter == 1 & subregion == `region' & (rent > 0 & rent <.), detail

	local anterent= r(N)
	
	qui sum lnrent [aw = weight] if renter == 1 & subregion == `region', detail
	local ameanrent = r(mean)
	local asdrent   = r(sd)
	
	* identify and delete outliers
	replace  rent = . if (abs((lnrent - `ameanrent') / `asdrent') > 2.5 & /*
		*/	 !missing(lnrent)) & subregion == `region' & renter == 1 

	count if !missing(rent) & subregion == `region'
	local postrent = r(N)
	loc rout = (`anterent' - `postrent')/`anterent'*100
	
	if `rout' > 5 & `rout' <. {
		disp in red "region: `region'  " as res round(`rout',.1) "%" as res " outliers"
	}
}
drop lnrent
...
levelsof subregion, local(regions)
foreach region of local regions {
	qui sum rent_self   [aw = weight] if renter == 0 & subregion == `region' & (rent_self > 0 & rent_self <.), detail
	local anterent= r(N)
	qui sum lnrent_self [aw = weight] if renter == 0 & subregion == `region', detail
	local ameanrent = r(mean)
	local asdrent   = r(sd)
	* identify and delete outliers
	replace  rent_self = . if (abs((lnrent_self - `ameanrent') / `asdrent') > 2.5 & ~mi(lnrent_self)) & subregion == `region' & renter == 0 
	count if !mi(rent_self) & subregion == `region'
	local postrent = r(N)
	loc rout = (`anterent' - `postrent')/`anterent'*100
	if `rout' > 5 & `rout' <. {
		disp in red "the regionsince is: `region'  "  " the variable is: rent_self    " as res round(`rout',.1) "%" as res " outliers"
	}
}
drop lnrent_self
...
gen     lnrent = ln(rent)      
replace lnrent = ln(rent_self) if renter == 0
local   ln_rent  = "lnrent"

* Step 2- Independent variables 
tab subregion, gen(regions) 

local var_rent hsize garage kitchen bedrooms area_dwl floor1 floor3 roof1 ///
roof2 type_hhd1 elect1 elect2 elect3 ///
garbage1 garbage2 garbage3 road_type1 road_type2 road_type3 road_type5 ///
cooler1 cooler3 regions1-regions5 regions7-regions9 urban
...
* Step 2- Independent variables 
local var_rent = "hsize garage kitchen bedrooms area_dwl floor1 floor3 roof1 roof2 type_hhd1 elect1 elect2 elect3 garbage1 garbage2 garbage3 road_type1 road_type2 road_type3 road_type5 cooler1 cooler3 regions1-regions5 regions7-regions9" 	
